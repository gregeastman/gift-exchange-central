    ...
    def get_gift_exchange_member(self):
        ...
        if session_user:
            try:
                ...
                #TODO: consider to ignore users that are google users with this email, as well as unverified users
            except:
                pass
...
class SignupHandler(MainWebAppHandler):
    ...
    def post(self):
        ...
        if not user_data[0]: #user_data is a tuple
            #TODO: use a more specific template
        ...
        self.add_template_values({'message': 'A verification email has been sent to your inbox'})
        self.render_template('message.html')
...
class SetPasswordHandler(MainWebAppHandler):
    ...
    def post(self):
        ...
        if not password or password != self.request.get('confirm_password'):
            self.add_template_values({'message': 'passwords do not match'})
            self.render_template('message.html')
            return
        user = self.user
        ...
        self.add_template_values({'message': 'Password updated'})
        self.render_template('message.html')
...
class ForgotPasswordHandler(MainWebAppHandler):
    ...
    def post(self):
        ...
        self.add_template_values({'message': 'An email with a link to reset your password has been sent'})
        self.render_template('message.html')
...
class PreferencesHandler(MainWebAppHandler):
    ...
    def get(self):
        ...
        #TODO: handle switching from native to google
        member = self.get_gift_exchange_member()
        template_values = {
